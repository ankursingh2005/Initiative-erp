(() => {
  ...
  function attendanceDashboard(){
    ...
    const options=(key,label)=>'<label><span class="attendance-sr-only">'+label+'</span><select data-attendance-filter="'+key+'"><option value="">'+label+'</option>'+[...new Set((attendanceSummary?.rows||[]).map(r=>r[key]).filter(Boolean))].sort().map(value=>'<option '+(attendanceFilters[key]===value?'selected':'')+' value="'+esc(value)+'">'+esc(value)+'</option>').join('')+'</select></label>';
    ...
    return '<section class="ems-attendance"><div class="attendance-banner"><div class="panel-head"><h2>EMS Attendance Dashboard</h2><span class="badge">Active EMS users</span></div><div class="attendance-filters"><label><span class="attendance-sr-only">Attendance date</span><input id="attendanceDay" type="date" max="'+today()+'" value="'+attendanceDay+'"></label>'+options('outlet','Outlets')+options('department','Departments')+'<button class="attendance-share" data-action="attendance-share">Share WhatsApp</button>'+options('weekoff_day','Week off')+'<label><span class="attendance-sr-only">Export month</span><input id="attendanceMonth" type="month" max="'+today().slice(0,7)+'" value="'+attendanceDay.slice(0,7)+'"></label><details class="attendance-download-menu"><summary>Download <span aria-hidden="true">&#9662;</span></summary><div class="attendance-download-options"><button type="button" data-action="attendance-download">Selected day CSV</button><button type="button" data-action="attendance-month">Selected month CSV</button></div></details></div></div><div class="attendance-body"><p class="subtext">'+date(attendanceDay)+' &middot; Active employees only &middot; Brand promoters excluded &middot; Times in IST</p><div class="attendance-metrics">'+cards.map(([label,status,count,kind])=>'<button type="button" class="attendance-metric '+kind+'" data-action="attendance-status" data-id="'+status+'" aria-pressed="'+(attendanceStatus===status)+'"><span>'+label+'</span><strong>'+count+'</strong></button>').join('')+'</div><button type="button" class="attendance-toggle" data-action="attendance-toggle" aria-expanded="'+attendanceExpanded+'" aria-controls="attendanceUsers">'+(attendanceExpanded?'Hide':'Show')+' '+(attendanceStatus||'all')+' employees ('+visible.length+') <span aria-hidden="true">'+(attendanceExpanded?'&#9650;':'&#9660;')+'</span></button><div id="attendanceUsers" '+(attendanceExpanded?'':'hidden')+'>'+table(['Employee','Status','Check in','Check out','Outlet','Department','Week off'],visible.map(r=>'<tr><td>'+esc(r.name)+'<small>'+esc(r.employee_id)+'</small></td><td><span class="attendance-status '+r.status.toLowerCase().replace(' ','-')+'">'+esc(r.status)+'</span></td><td>'+time(r.check_in)+'</td><td>'+time(r.check_out)+'</td><td>'+esc(r.outlet)+'</td><td>'+esc(r.department)+'</td><td>'+esc(r.weekoff_day||'?')+'</td></tr>'),'No employees match these filters.')+'</div></div></section>';
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='attendance-download'){b.closest('details')?.removeAttribute('open');downloadAttendanceCsv(filteredAttendance().filter(r=>!attendanceStatus||r.status===attendanceStatus),'ems-attendance-'+attendanceDay+'.csv');return;}
      if(action==='attendance-month'){b.closest('details')?.removeAttribute('open');const month=$('attendanceMonth').value;if(!month||!$('attendanceMonth').checkValidity())return;b.disabled=true;const result=await api('/attendance-summary?month='+month);downloadAttendanceCsv(filteredAttendance(result.rows),'ems-attendance-'+month+'.csv');return;}
