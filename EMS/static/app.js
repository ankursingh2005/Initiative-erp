(() => {
  ...
  function overviewPage(){
    ...
    const metrics=manager()?[stat('ACTIVE EMPLOYEES',activeEmployeeIds.size,'Across your organisation'),stat('PENDING APPROVAL',employees.filter(e=>e.status==='pending').length,'New employee registrations'),stat('PRESENT TODAY',presentToday,'Active employee check-ins'),stat('LEAVE REQUESTS',pending.length,'Awaiting a decision')]:[stat('DAYS PRESENT',attendance.filter(a=>a.day.startsWith(month)).length,'This calendar month'),stat('LEAVE REQUESTS',pending.length,'Awaiting HR approval'),stat('LATEST NET PAY',payroll.length?money(payroll[0].net):'—','Latest published payslip'),stat('REWARDS',money(earnings.filter(e=>e.month===month).reduce((s,e)=>s+e.amount,0)),'Approved this month')];
