...
(function(){const role=localStorage.getItem('role')||'',token=localStorage.getItem('token');if(!token)return;const base=location.pathname==='/erp'||location.pathname.startsWith('/erp/')?'/erp':'',route=location.pathname.slice(base.length).replace(/\.html$/,'')||'/';const unrestricted=['Admin','Owner','HR','MISExecutive','Accounts'],schemeRoles=[...unrestricted,'CategoryManager','BrandManager'],publicRoutes=['/','/login','/signup','/forgot-password','/reset-password','/privacy','/offline'],basicRoutes=['/home','/attendance',...(role!=='BrandPartner'?['/identity-card']:[])];if(!publicRoutes.includes(route)&&!basicRoutes.includes(route)&&!(route==='/dashboard'&&schemeRoles.includes(role))&&!(route==='/purchase-orders'&&role==='CategoryManager')&&!unrestricted.includes(role)){location.replace((base||'')+'/home');return}function trimMobileNav(){if(['BrandPartner','SupportingStaff'].includes(role))document.querySelector('.app-mobile-nav a[href$="/dashboard"]')?.remove()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',trimMobileNav,{once:true});else trimMobileNav()})();
