    ...
    const DIVISION_LABELS = { HA: 'Home Appliances', HE: 'Home Entertainment', MH: 'Mobile', IT: 'Computer', DC: 'Digital Camera', ACC: 'Accessories', PAYOUT: 'Payouts', UNCATEGORIZED: 'Needs review', EXCLUDED: 'Excluded / unrelated' };
      ...
      body.innerHTML = rows.map(source => {
        ...
        return `
          <tr class="${rowClass}" data-row-id="${r.row_id}">
            ...
            <td>${esc(r.brand || '')}</td>
            <td>${esc(r.store || 'Unknown')}<br><small>${esc(r.vch_no || '')}</small></td>
    ...
    function renderKpis(data) {
      ...
      grid.innerHTML = `
        ...
        <div class="kpi"><div class="label">Margin on sales</div><div class="value ${k.margin_percent >= 0 ? 'good' : 'bad'}">${pct(k.margin_percent)}</div></div>
        <div class="kpi"><div class="label">Included product lines</div><div class="value">${k.transactions.toLocaleString('en-IN')}</div></div>
    ...
    function renderMarginScenario(){
      const d=lastDashboardData?.decisions;
      if(!d) return;
      const input=document.getElementById('marginLift'), lift=Number(input.value);
      document.getElementById('marginScenario').textContent=input.value==='' || !Number.isFinite(lift) || lift<0 || lift>20
        ? 'Enter a margin increase from 0 to 20 percentage points.'
        : d.sales<=0 ? 'A scenario needs positive net sales.'
        : 'Additional gross profit: '+inr(d.sales*lift/100)+'. Illustrated total gross profit: '+inr(d.profit+d.sales*lift/100)+'.';
    }
    function renderDecisions(data){
      const d=data.decisions||{}, rate=v=>v==null?'Not available':pct(v);
      document.getElementById('decisionKpis').innerHTML=[['Gross profit',inr(d.profit)],['Margin on sales',rate(d.margin)],['Markup on cost',rate(d.markup)],['Identified bills',d.bill_count||0],['Average positive bill',d.average_bill==null?'Not available':inr(d.average_bill)],['Returns / credits',inr(d.returns)]].map(([label,value])=>'<div class="kpi"><div class="label">'+label+'</div><div class="value">'+value+'</div></div>').join('');
      document.getElementById('decisionCoverage').textContent=(d.missing_bill_rows||0)+' lines lack bill numbers and are excluded from bill metrics. Average bill uses '+(d.positive_bill_count||0)+' bills with positive net sales within the current filters. Margin = gross profit / sales; markup = gross profit / cost (the source Profit % basis).';
      document.getElementById('decisionMatrix').innerHTML=(d.matrix||[]).map((r,i)=>'<tr><td><button class="btn btn-ghost" data-matrix-row="'+i+'">'+esc(r.store)+'</button></td><td>'+esc(r.category_name)+'</td><td class="num">'+inr(r.sales)+'</td><td class="num">'+inr(r.profit)+'</td><td class="num">'+rate(r.margin)+'</td><td class="num">'+r.bills+'</td></tr>').join('');
      document.querySelectorAll('[data-matrix-row]').forEach(button=>button.addEventListener('click',()=>{
        const r=d.matrix[Number(button.dataset.matrixRow)];
        document.getElementById('storeFilter').value=r.store;
        document.getElementById('divisionFilter').value=r.category;
        refreshDashboard();
      }));
      document.getElementById('decisionLossNote').textContent='Showing the largest '+(d.loss_lines||[]).length+' of '+(d.loss_line_count||0)+' positive-sales lines below cost. Total loss on these sales: '+inr(d.selling_loss)+'. Returns are excluded from this list. Use the filtered CSV for complete detail.';
      document.getElementById('decisionLosses').innerHTML=(d.loss_lines||[]).map(r=>'<tr><td>'+esc(r.date)+'</td><td>'+esc(r.store)+'</td><td>'+esc(r.voucher||'Missing')+'</td><td>'+esc(r.item)+'</td><td class="num">'+inr(r.sales)+'</td><td class="num">'+inr(r.loss)+'</td></tr>').join('') || '<tr><td colspan="6">No below-cost positive-sales lines in this selection.</td></tr>';
      renderMarginScenario();
    }
