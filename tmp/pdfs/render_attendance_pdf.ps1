...
$pdfPath = Join-Path (Get-Location) 'output/pdf/Attendance_User_Guide.pdf'
...
for($i=0; $i -lt $pdf.PageCount; $i++) {
    $page = $pdf.GetPage($i)
    $outFile = Await-Result ($folder.CreateFileAsync("attendance-page-$($i+1).png", [Windows.Storage.CreationCollisionOption]::ReplaceExisting)) ([Windows.Storage.StorageFile])
