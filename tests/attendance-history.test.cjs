...
test('employee history loads by permanent ID after a rename, with older dates available',async()=>{
  ...
  const calls=[];
  let created=0;const context={document:{createElement:()=>created++===0?modal:{append(){},appendChild(){},after(){},setAttribute(){},replaceChildren(){}},body:{appendChild(){}}},today:'2026-09-21',
  ...
  assert.match(detail.innerHTML,/Punch In - 09:00/);
  assert.match(detail.innerHTML,/Punch Out - 18:00/);
  assert.doesNotMatch(detail.innerHTML,/record-stat|Working hours/);
  assert.match(detail.innerHTML,/photo-99/);
  // A slower response for a previously selected date must not replace the latest date.
  let resolveOld;
  context.fetchSelfieRecord=id=>id==='99'?new Promise(resolve=>{resolveOld=resolve}):Promise.resolve({checkin_selfie:'latest-photo'});
  const oldRequest=select.onchange();
  select.value='100';await select.onchange();
  resolveOld({checkin_selfie:'old-photo'});await oldRequest;
  assert.match(detail.innerHTML,/latest-photo/);
  assert.doesNotMatch(detail.innerHTML,/old-photo/);
});
